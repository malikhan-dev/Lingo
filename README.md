...
in some scenario and test cases (the 50,000,000 in-memory records filtering and validation) it can be faster than C# LINQ (Not Guaranteed). and its 100% type safe with zero reflections (unless sifu expressions is used). for collection processing it performs compiled queries in a single execution unit (operator fusion pattern). 
...
### Intro
ZenQL (Zen-QL) is an internal query language for Go, designed to help you filter, search, validate, process, and stream data with readability and ease. Inspired by LINQ in C# and Java Streams, ZenQL brings the power of polymorphic querying to the Go ecosystem while adhering to idiomatic Go practices.
...
### WhereEx:
Accepts an expressions generated by Sifu Expressions.

``` go

collections.From(&items).WhereEx(expr.Prop("Flag").collect()
...
- a less function expression generated by Sifu.Expressions
- 
- desc bool: determinse the sort direction, ascending or descending
...
1 - an update Expression (using sifu expressions).
